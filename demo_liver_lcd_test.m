...
if(small_dataset)
    ...
    dose = [100]; % one dose level 100%.
    all_recon_type = {'fbp_sharp', 'redcnn'} %reconstruction folder names 
    n_sp = 10; % number of signal-present cases 
...
n_reader = 15; % Number of readers (times of data re-split into training/testing for variance estimation.)
...
aucmean =  squeeze(mean(auc_all));   %mean of AUCs
aucse = squeeze(std(auc_all))/sqrt(n_reader); % standard errors of AUCs
snrmean =  squeeze(mean(snr_all));   %mean of detectability SNRs
snrse = squeeze(std(snr_all))/sqrt(n_reader); % standard errors of detectability SNRs
